...
async def create_products_table_if_missing() -> None:
        ...
        async with engine.begin() as conn:
            await conn.run_sync(
                lambda sync_conn: Base.metadata.create_all(
                    sync_conn,
                    tables=[ProductModel.__table__],
                    checkfirst=True,
                )
            )
        print("Tabla 'products' verificada/creada correctamente.")
    finally:
        await engine.dispose()


def main() -> None:
    asyncio.run(create_products_table_if_missing())
