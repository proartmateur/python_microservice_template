"""Crea la tabla de User y su indice parcial keyset.

Uso:
    uv run poe create_users_table
...
async def main() -> None:
    ...
    if settings.REPOSITORY_DATA_SOURCE != "database":
        raise RuntimeError(
            "create_users_table requiere REPOSITORY_DATA_SOURCE=database."
        )
    ...
    async with db_manager.engine.begin() as conn:
        await conn.run_sync(
            ...
            tables=[UserModel.__table__],  # type: ignore[list-item]
        )
    ...
    print("Tabla users e indice parcial creados correctamente.")
